!function(g){
    ...
    if(!canvas.getContext){console.log("浏览器版本过低！");return false;}
    ...
    function Circle(background) {
      ...
      this.opacity=this.opacity<=0.1?0.2:this.opacity;
    ...
    function drawCircle(ctx, circle) {
      ...
      if (circle.filled == 'full') {
        ctx.fillStyle = ['rgba(', circle.borderColor, ',',circle.opacity *0.2, ')'].join('');
    ...
    function draw() {
      ...
      var ctxfr = document.getElementById('line-canvas').getContext('2d');
      var ctxbg = document.getElementById('line-bg').getContext('2d');
        ...
        for (var i = 0; i < arr.length - 1; i++) {
          for (var j = i + 1; j < arr.length; j++) {
            ...
            if (dist < linkDist) {
              ...
              ctx.strokeStyle = ["rgba(", arr[i].borderColor, ",", 0.1, ")"].join("");
