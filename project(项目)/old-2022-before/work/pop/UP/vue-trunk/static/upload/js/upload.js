$.fn.bindUpload = function () {
    var myData = {
		editor:arguments[0].editor|| "",
		layer:arguments[0].layer || "",
		layindex:arguments[0].layindex || "",
        url: arguments[0].url || "",
	...
	_this.on("click",".insert",function(){
		var img_num = 0;
		for(var i in status){
			if(status[i].state ==0){
				img_num++;
		...
		if(upimages.length == 0){
			myData.layer.alert("没有上传完成的图片！");
			return;
		}
		if(img_num>upimages){
			myData.layer.alert("请删除上传失败的图！");
			return;
		}
		for(var j in upimages){
			myData.editor.insertContent('<img src="'+upimages[j]+'">');
		}
		myData.layer.closeAll();
	});

    input.bind("change", function () {
		upimages = [];
		if(this.files.length>myData.num){
		    console.log(5515);
            myData.layer.alert("一次最多上传"+myData.num+"张图片，超出自动剔除！");
        }
        if($(this).val()!="") {
