...
<template>
  <div class="app">
    ...
    <p style="text-align: center">
      Edit
      <code>src/api/*.ts</code> and watch it change.
      <br />You can also open Devtools to see the request details.
    </p>
    <div>
      <p>
        <span class="lambda">λ GET</span>
        <span class="lambda">getDate()</span>
        <span>Server Date: {{ date }}</span>
      </p>
      <p>
        <span class="lambda">λ POST</span>
        <span class="lambda">fetchStars('midwayjs/midway')</span>
        <span>Github Stars: {{ repo?.stars }}</span>
      </p>
      <p>
        <span class="lambda">λ GET</span>
        <span class="lambda">getBookByParams({ params: { id: '1' } })</span>
        <span>Book title: {{ bookByParams?.title }}</span>
      </p>
      <p>
        <span class="lambda">λ GET</span>
        <span class="lambda">getBookByQuery({ query: { id: '2' } })</span>
