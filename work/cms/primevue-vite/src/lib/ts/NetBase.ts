...
export default class NetBase {
  ...
  constructor(config: NetBaseConfig) {
    for (let key in config) {
      this.config[key as keyof NetBaseConfig] =
        config[key as keyof NetBaseConfig];
    }
  }
  ...
  private static getParams(obj: NetParams): AnyObject {
    var param: AnyObject = {};
    for (var key in obj) {
      if (!["_success", "_fail", "_complete"].includes(key))
        param[key] = obj[key];
    }
    return param;
  }
          ...
          .then((res) => {
            if (res.status === 200) {
              res
                .json()
                .then((json) => {
                  if (baseConfig?.baseSuccess)
                    json = baseConfig.baseSuccess(json) || json;
                ...
                .catch((err) => {
                  if (baseConfig?.canNoJson) {
                    res
                      ...
                      .then((text) => {
                        if (baseConfig?.baseSuccess)
                          baseConfig.baseSuccess(text);
                        if (baseConfig?.baseComplete)
                          baseConfig.baseComplete(text);
                        if (params._success) params._success(text);
                        if (params._complete) params._complete(text);
                        resolve(text as any);
                      })
                      .catch((err) => {
                        if (baseConfig?.baseFail)
                          err = baseConfig.baseFail(err) || err;
                        if (baseConfig?.baseComplete)
                          baseConfig.baseComplete(err);
