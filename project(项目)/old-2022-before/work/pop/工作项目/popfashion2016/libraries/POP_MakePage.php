...
class POP_MakePage
{
    ...
    public function echoPageAsDiv()
    {
        ...
        if (!empty($pageList)) {
            if ($this->pageCount > 1) {
                $keyword = get_instance()->input->get('key', TRUE);
                $keywords = $keyword ? '?key=' . rawurlencode(rawurlencode($keyword)) : '';
                if ($this->hasPrePage) {
                    $pageString = $pageString . "<a class='prevPage' href=\"" . $this->url . "page_" . ($this->pageNo - 1) . $this->urlsuffix . $keywords . "#anchor\"><em>&lt;</em>上一页</a>";
                }
                foreach ($pageList as $k => $p) {
                    if ($this->pageNo == $p) {
						$sp_class = $this->pageNo==1?" page-border-left":"";
						$sp_class = $this->pageNo==$this->pageCount?" page-border-right":$sp_class;
                        $pageString = $pageString . "<span class='hov{$sp_class}'>" . $this->pageNo . "</span>";
                        continue;
                    }
                    if ($p == -1) {
                        $pageString = $pageString . "<span class='ellipsis'>...</span>";
                        continue;
                    }
                ...
                if ($this->hasNextPage) {
                    $pageString = $pageString . "<a class='nextPage' href=\"" . $this->url . "page_" . ($this->pageNo + 1) . $this->urlsuffix . $keywords . "#anchor\">下一页<em>&gt;</em></a>";
                }
