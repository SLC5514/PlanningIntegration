 ...
 (function ($, window, document, undefined) {
  ...
  Plugin.prototype = {
    init: function () {
      ...
      if ($(this.element).next().is('canvas')) $(this.element).next().remove();
      $(this.element).after(scope.canvas);
      scope.ctx = scope.canvas.getContext("2d");
      scope.width = this.options.width || $(this.element).width();
      scope.height = this.options.height || $(this.element).height();
      ...
      scope.ctx.drawImage(this.image[0], 0, 0, scope.width, scope.height);
      $(this.element).hide();
