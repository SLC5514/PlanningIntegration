 ...
 *
 * @property-read  VirtualTryOn_model $VirtualTryOn_model
 */
class VirtualTryOn extends POP_Controller
{
    ...
    public function __construct()
    {
        ...
        //后台预览和分享列表跳过权限验证
        if ((!empty($_REQUEST['hash']) && in_array(strtolower($method), ['getvirtualdetail'])) || in_array(strtolower($method), ['sharelist', 'managegroup', 'savemyshareset'])) {
    ...
    public function templateSelectShow()
    {
        ...
        $template_site = $this->VirtualTryOn_model->getUserTplSite($aUser['id']);
        if ($this->get_use_type == "GENERAL" && !$template_site) {
            //弹出弹窗选择
    ...
    public function getLeftMenu($param = '')
    {
        ...
        if ($this->get_use_type == "GENERAL") {
            ...
            foreach ($iTplSite as $site) {
                ...
                foreach ($classify as $value) {
                    ...
                    $classify_names[] = ['id' => $value['id'], 'name' => $value['sName']];
                }
        ...
        } elseif (in_array($this->get_use_type, ["VIP", "TRIAL"])) {//VIP
    ...
    public function getVirtualDetail()
    {
        ...
        if ($this->input->is_ajax_request()) {
            outPrintApiJson($code, 'OK', $detail);
        } else {
