...
$(function(){
    ...
    function setDom(data){
        ...
        for(var i=0,len=arr.length;i<len;i++){
            ...
            var virtualUrl = arr[i]["virtualUrl"]?arr[i]["virtualUrl"]:null;
            if(def.col == 82 || def.col == 124){
                _html+='<li>';              
                _html +='<div itemscope class="pic_box">';
                if(P_UserType==5){
                    _html+='<a class="pic-item-area js-general-user-status" href="javascript:void(0)" ondragstart="return false;" oncontextmenu="return false;" title="查看详情">';
                }else{
                    _html+='<a itemprop="url" class="img_click js-data-collect" href="/details/style/t_'+tableName+'-id_'+id+'-col_'+columnId+'/" data-t="'+tableName+'" data-col="'+columnId+'" data-id="'+id+'" data-total="'+total+'" data-index="'+offset+'" ondragstart="return false;" oncontextmenu="return false;" title="查看详情">';
                }                    
                ...
                if(i<5){
                    ...
                    _html+='<img alt="'+title+'" itemprop="thumbnail" src="https://imgf2.pop-fashion.com/global/images/loading/patterns270.jpg" data-original="'+cover+'">';
                }
                _html+='<div class="color-time-box">';
                _html+='<p class="time"><em></em>'+create_time+'</p>';
                if(colorProportion && colorProportion.length>0 && def.col == 82){
                    _html+='<div class="color-darea clearfix">';
                    for(var k=0,clen=colorProportion.length;k<clen;k++){                       
                        _html+='<p style="background:'+colorProportion[k].Color+';width:'+colorProportion[k].Percent+'%;" title="潘通参考值:'+colorProportion[k].pantonColorNumber+'"></p>';
                    }
                    _html+='</div>';
                }
                _html+='</div>';
                _html+='</a>';
                if(def.col == 82){
                _html+='<a class="mn-cloth-btn js-mn-cloth-btn" href="'+virtualUrl+'" target="_blank" title="模拟成品"><i></i>模拟成品</a>';
                }
                _html+='</div>';
                if(P_Collect){
                    if(iCollectStatus ==1){
                        _html+='<a class="patter-collect-btn js-collect-btn cur" href="javascript:void(0);"></a>';
                    }else{
                        _html+='<a class="patter-collect-btn js-collect-btn" href="javascript:void(0);"></a>';
                    }
                }
                if(def.col == 82){
                    _html+='<div class="titleBox titleBox-left">';
                    _html+='<div class="patter-label-position patter-left-label-position">'
                }else{
                    _html+='<div class="titleBox">';
                    _html+='<div class="patter-label-position">'
                }
                for(var a=0, alen=labels.length;a<alen;a++){
                    _html+='<a class="label-link" href="'+labels[a].lLink+'">'+labels[a].name+'</a>'
                }
                _html+='</div>';
                _html+='</div>';
                _html+='</li>';
                ...
                _html+='<li class="pattern-item-li">';
                _html+='<div itemscope class="pic_box">';
                if(P_UserType==5){
                    _html+='<a class="pic-item-area js-general-user-status" href="javascript:void(0)" ondragstart="return false;" oncontextmenu="return false;" title="查看详情">';
                }else{
                ...
                // }
                if(i>5){
                    _html+='<img alt="'+title+'" itemprop="thumbnail" src="https://imgf2.pop-fashion.com/global/images/loading/referenceMR150.jpg" data-original="'+cover+'">'
                }else{
                    _html+='<img alt="'+title+'" itemprop="thumbnail" src="'+cover+'">';
                }
                _html+='</a>';
    ...
    // 标签显示全部
    $(".js-patterns-lists-box").on('mouseenter mouseleave','li',function(e){
        if($(".patter-left-label-position").length==0){
            return false;
        }
        var label_position=$(this).find(".patter-left-label-position")
        ...
        if(e.type=="mouseenter"){
            ...
            label_position.animate({height:'30px'},50);
        }
    });
    // 没有数据
