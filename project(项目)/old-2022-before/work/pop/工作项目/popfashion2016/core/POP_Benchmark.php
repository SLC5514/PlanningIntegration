		...
		if (BENCHMARK) {
			...
			if (( ! empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest')) {
				if (substr($start, -3) === 'End') {
                    ...
                    //$logPath = dirname(getenv('BASEPATH')) . '/popfashion2016/logs/';
					$logPath = dirname(dirname(dirname(__FILE__))) . '/popfashion2016/logs/';
			...
			// 否则直接输出
				...
				if (substr($start, -3) === 'End') {
                    ...
                    $this->elapsed[] = $_start . '到' . $start . '执行时间：' . $time . '秒';
				}
			}
		}
	}
