...
$(function(){
    ...
    def.cut_ow=def.ow<=1200?528:720;
    ...
    function setDom(data){
        ...
        for(var i=0,len=arr.length;i<len;i++){
            ...
            var title=arr[i]["list"]["sTitle"]?arr[i]["list"]["sTitle"]:"";
            ...
            var iCollectStatus =arr[i]["iCollectStatus"]?arr[i]["iCollectStatus"]:0;
            var isShowMark = arr[i]["isShowMark"]?arr[i]["isShowMark"]:0;
            var iFabricPublish =arr[i]["list"]["iFabricPublish"]?arr[i]["list"]["iFabricPublish"]:"";
            var description = arr[i]["list"]["description"]?arr[i]["list"]["description"]:"";

           _html+='<li class="">';
            _html+='<a itemprop="url" href="/details/report/t_'+tableName+'-id_'+id+'-col_'+columnId+'/" target="_blank" data-t="'+tableName+'" data-id="'+id+'" data-total="'+total+'" data-index="'+offset+'" data-col="'+columnId+'" data-mark="'+isShowMark+'" data-utype="" class="trend-a js-data-collect" ondragstart="return false;" oncontextmenu="return false;" title="查看详情">';
            // if(cover.length >30){
            //     _html+='<img alt="'+title+'" itemprop="thumbnail" src="https://imgf2.pop-fashion.com/global/images/loading/analysis.jpg" data-original="'+cover+'">';
            // }else{
            //     _html+='<img itemprop="thumbnail" src="https://imgf2.pop-fashion.com/global/images/loading/analysis.jpg" alt="pic">'
            // }
            if(i>4){
                _html+='<img alt="'+title+'" itemprop="thumbnail" src="https://imgf2.pop-fashion.com/global/images/loading/analysis.jpg" data-original="'+cover+'" alt="pic">';
            }else{
                _html+='<img alt="'+title+'" itemprop="thumbnail" src="'+cover+'" alt="pic">';
            }
            _html+='<div class="img_arrow_icon">';
            if(isShowMark == 1){
                _html+='<span class="arrow_icon">最热门</span>';
            }
            if(iFabricPublish && iFabricPublish ==1){
                _html+='<span class="ulb_icon">配面料</span>';
            }
            _html+='</div>';
            _html+='<p itemscope class="time clearfix">';
            _html+='<span itemprop="release time" class="fl"><img itemprop="thumbnail" src="https://imgf2.pop-fashion.com/global/images/zzjd_02.png" alt="创建时间">'+create_time+'</span>'
            _html+='</p>';
            _html+='</a>';
            if(P_Collect){
                if(iCollectStatus == 1){
                    _html+='<a class="report-collect-btn js-collect-btn cur" href="javascript:void(0);"></a>';
                }else{
                    _html+='<a class="report-collect-btn js-collect-btn" href="javascript:void(0);"></a>'
                }
            }
            _html+='<div itemscope class="text-down">';
            _html+='<p itemprop="name" class="title topic-title" title="'+title+'">'+title+'</p>';
            _html+='<p class="img-description topic-img-description js-topic-description">'+pop_fashion_global.fn.cutByWidth(description,def.cut_ow,12)+'</p>'
            _html+='</div>';
            _html+='</li>';
        }
        if(_html!=''){
            if(def.get_times==1){
                $('.js-patterns-lists-box .js-placeholder-item').hide();
                tag.append(_html);
            ...
            $('.js-patterns-lists-box img').lazyload({effect: "show"});
    ...
    function resetPage(){                                                                                       //新页重置dom
        ...
        $('.js-patterns-lists-box>ul>li').each(function(){
