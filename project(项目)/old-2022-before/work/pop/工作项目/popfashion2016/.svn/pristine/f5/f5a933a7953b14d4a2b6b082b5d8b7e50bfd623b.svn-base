...
var Birds = function () {
    ...
    /*****************************
		...
		*****************************/
		...
		for(var i = 0; i < 200; i ++) {
			...
			_bird = _birds[ i ] = new THREE.Mesh( new Bird(), new THREE.MeshBasicMaterial( { color:Math.random() * 0x0000FF, side: THREE.DoubleSide } ) );
	...
	this._render = function() {
		for ( var i = 0, il = _birds.length; i < il; i++ ) {
			...
			color.r= ( 1379 - _bird.position.z ) / 1000;
			color.g = ( 1324 - _bird.position.z ) / 1000;
			color.b = ( 500 - _bird.position.z ) / 1000;
